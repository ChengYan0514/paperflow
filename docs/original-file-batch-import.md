...
`openalex/original/{source_id}/{file_name}` 文件。CSV 使用 UTF-8（可带 BOM）和标准逗号格式。
...
`original_file` 的 `file_id` 由 `source_id + year + paper_title + authors` 的规范化稳定 SHA-256 哈希生成，CSV 的 `file_name` 去掉扩展名后必须等于该值；同类型跳过，PDF/XML/HTML 按 PDF > XML > HTML 升级，
