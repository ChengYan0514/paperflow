...
import { Button, Form, Input, InputNumber, Space, Spin, Upload, message } from 'antd';
...
export default function PaperFormPage() {
  ...
  return (
    <PageContainer title={editing ? '编辑论文' : '导入论文'}>
      <Spin spinning={loading}>
        <Form<FormValues> form={form} layout="vertical" onFinish={submit} initialValues={{ authors: [''] }} style={{ maxWidth: 820 }}>
          <Form.Item name="sourceId" label="OpenAlex 来源" rules={[{ required: true }]}>
            <SourceSearchSelect disabled={matched} />
          </Form.Item>
          <Form.Item name="year" label="发表年份" rules={[{ required: true }]}>
            <InputNumber min={1000} max={new Date().getFullYear() + 1} style={{ width: '100%' }} disabled={matched} />
          </Form.Item>
          <Form.Item name="paperTitle" label="论文标题" rules={[{ required: true }, { max: 2000 }]}>
            <Input.TextArea autoSize={{ minRows: 2, maxRows: 5 }} disabled={matched} />
          </Form.Item>
              ...
              <Form.Item label="作者">
                {fields.map((field, index) => (
                  <Space key={field.key} align="baseline" style={{ display: 'flex', marginBottom: 8 }}>
                    <Form.Item {...field} rules={[{ required: true, message: '请输入作者姓名' }]} noStyle>
                      <Input placeholder={`作者 ${index + 1}`} disabled={matched} />
                    </Form.Item>
                    {!matched ? <Button type="text" onClick={() => move(index, Math.max(0, index - 1))}>上移</Button> : null}
                    {!matched ? <MinusCircleOutlined onClick={() => remove(field.name)} /> : null}
                  </Space>
                ))}
                {!matched ? <Button type="dashed" onClick={() => add()} icon={<PlusOutlined />}>添加作者</Button> : null}
              </Form.Item>
            )}
          </Form.List>
          <Form.Item name="doi" label="DOI"><Input disabled={matched} /></Form.Item>
          <Form.Item name="url" label="原始文章 URL" rules={[{ type: 'url' }]}><Input /></Form.Item>
          {!editing ? (
          ...
          <Space>
            <Button type="primary" htmlType="submit">{editing ? '保存修改' : '导入论文'}</Button>
