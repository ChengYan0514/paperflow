...
本文定义第一版只读前端管理页面的制作计划。前端只消费 Java Admin REST API，
...
- 登录、角色、权限、JWT、Session。
- 任何写操作，包括状态重置、人工修正匹配、重试任务。
- 直接连接 PostgreSQL。
...
1. 创建 `web-admin/` Vite React TypeScript 项目骨架。
2. 实现 API client、分页类型、错误响应处理。
3. 实现基础布局：左侧导航、顶部当前页面标题、内容区。
4. 实现工作台、来源期刊列表和详情。
5. 实现 Works 列表、详情、blocks 页面。
6. 实现 Original Files 列表和详情。
7. 补充 loading、empty、error 状态。
8. 用本地 Java 后端做手工验收。
...
- 没有写接口、登录、缓存、直接文件读取、pipeline 触发代码。
