...
| `/fields` | `/knowledge/causal-graph/fields` | 都按子领域/主题分析。原项目是 Top 10×10 热力图，选领域后展示方法占比、变量、关系；当前是可排序、分页的完整聚合表，并可按子领域、主题筛选。 |
| `/methodology` | `/knowledge/causal-graph/methodology` | 都说明重复度、方法多样性、方向分歧等概念。原项目是面向研究者的完整方法论、公式和局限；当前是面向运维/管理的数据库字段与聚合口径说明。 |
