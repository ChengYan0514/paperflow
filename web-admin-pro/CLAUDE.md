...
- **Requests**: use Umi `request` from `@umijs/max`; keep backend base URL in `process.env.API_BASE_URL`.
...
**State**: `useModel('filename')` for global hooks (`src/models/`). `useModel('@@initialState')` for currentUser/settings. ProTable `request` prop or local state around service calls for data loading.
